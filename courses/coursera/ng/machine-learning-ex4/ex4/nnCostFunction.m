...
a1 = [ones(m, 1), a1];  % m x (input_layer_size + 1)
...
y_matrix = all_combos(y, :); % m x num_labels

t1 = Theta1(:, 2:end);
t2 = Theta2(:, 2:end);

J = - 1.0 / m * sum(sum(y_matrix .* log(h) + (1 - y_matrix) .* log(1 - h))) + ... 
    lambda / (2 * m) * (sum(sum(t1 .^ 2)) + sum(sum(t2 .^ 2)));
